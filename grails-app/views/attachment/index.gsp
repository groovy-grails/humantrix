		...
		<div id="list-attachment" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="oraName" title="${message(code: 'attachment.oraName.label', default: 'Ora Name')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'attachment.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="type" title="${message(code: 'attachment.type.label', default: 'Type')}" />
				...
				<g:each in="${attachmentInstanceList}" status="i" var="attachmentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${attachmentInstance.id}">${fieldValue(bean: attachmentInstance, field: "oraName")}</g:link></td>
					
						<td>${fieldValue(bean: attachmentInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: attachmentInstance, field: "type")}</td>
